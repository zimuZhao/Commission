...
function changeDateTime(start, end) {
    ...
    getWorldSales(startTime, endTime);
}
            ...
            if (data.status) {
                if (data.result != "[]") {
                    ...
                    $.each(data.result.datas, function (index, item) {
                        ...
                        titemnode = titemnode.replace("{salesman}", item.name);
                        titemnode = titemnode.replace("{area}", item.pv);
                        titemnode = titemnode.replace("{locks}", (item.rpv * 100).toFixed(2));
                        titemnode = titemnode.replace("{stocks}", item.uv);
                        titemnode = titemnode.replace("{barrels}", item.vt);
                        titemnode = titemnode.replace("{sale}", item.vt);
